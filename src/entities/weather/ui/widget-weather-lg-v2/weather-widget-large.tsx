...
export const WeatherWidgetLargeV2 = React.memo(({ icon, temp, weatherDescription, cityName, feelsLike, tempMax, tempMin, windSpeed, humidity, country }: WeatherInfo) => {
	const date = new Date();
			...
			>
				<Box textAlign='left'>
					<Typography
						...
						variant='inherit'
					>
						Мин: {formattedTempMin}&deg;
					</Typography>
					<Typography
						className='max_temp'
						variant='inherit'
					>
						Макс: {formattedTempMax}&deg;
					</Typography>
				</Box>
				<Box textAlign='left'>
					<Typography
